...
void drawField(int x, int y, int w, int h, const char *label, uint16_t col=lnCol)
{
  lcd.drawRect(x,y+7,w,h-7,col);
  setInfoFont();
  font.setScale(1);
  font.setColor(labCol,BLACK);
  int wl = strlen(label); // font.strWidth(label);
...
void constData()
{
  drawField(0,0,140-5,35-2," RREFont Demo ",lnCol);
  drawField(0,35,140-5,35-2," 2nd line demo ",lnCol);
