...
/*  encodeur ---- les 2 phases génèrent des interruptions ----
 *  pour ne pas voir les rebonds, 
 *  les 4 transitions génèrent une interruption (chaque phase alternativement)
 *  au changement de direction, 
 *  c'est la phase inactive qui effectue la première transition.
 *  Celle-ci est perdue.
 *  
#define COD_CLK 17
#define COD_DAT 18
...
#define COD_VCC 20
#define COD_GND 21
#define COD_STA COD_CLK
#define COD_INT COD_DAT
*/
#define COD_CLK 21
#define COD_DAT 20
#define COD_SW  19
#define COD_VCC 18
#define COD_GND 17
#define COD_STA COD_DAT
#define COD_INT COD_CLK

#define COD_INT1 COD_DAT
#define COD_INT2 COD_CLK
...
uint8_t corpsw[]={6,12,18};
uint8_t corpsh[]={8,16,24};
uint8_t textsize=3;
...
void clrscr()
{
 ...
 tft.drawRect(posrectx,posrecty,wrect, hrect, WHITE);        
 tft.fillRect(posrectx+1,posrecty+1,wrect-2, hrect-2, BLACK);
}

/*void codIsr()
{
  if(digitalRead(COD_STA)==digitalRead(COD_INT)){codeur++;}else{codeur--;}
}*/

void codIsr1R()
{
  detachInterrupt(digitalPinToInterrupt(COD_INT1));
  if(digitalRead(COD_INT2)==HIGH){
    Serial.print(codeur);Serial.println(" -1");
    codeur--;
    attachInterrupt(digitalPinToInterrupt(COD_INT2), codIsr2F, FALLING);}
  else {
    Serial.print(codeur);Serial.println(" +1");
    codeur++;
    attachInterrupt(digitalPinToInterrupt(COD_INT2), codIsr2R, RISING);}
}
...
void codIsr2R()
{
  detachInterrupt(digitalPinToInterrupt(COD_INT2));
  if(digitalRead(COD_INT1)==HIGH){
    Serial.print(codeur);Serial.println(" +1");
    codeur++;
    attachInterrupt(digitalPinToInterrupt(COD_INT1), codIsr1F, FALLING);}
  else {
    Serial.print(codeur);Serial.println(" -1");
    codeur--;
    attachInterrupt(digitalPinToInterrupt(COD_INT1), codIsr1R, RISING);}
}
...
void setup() {
    ...
    pinMode(COD_CLK,INPUT_PULLUP);
    pinMode(COD_DAT,INPUT_PULLUP);
    pinMode(COD_GND,OUTPUT);
    digitalWrite(COD_GND,LOW);
    pinMode(COD_VCC,OUTPUT);
    digitalWrite(COD_VCC,HIGH);    

    wid=tft.width();
    ht=tft.height();
    posrectx=50;
    posrecty=120;
    wrect=wid-posrectx*2;
    hrect=ht-posrecty*2;
...
//    ID = 0x9329;                             // force ID
    ...
    clrscr();
    tft.setCursor(corpsw[textsize-1],corpsh[textsize-1]+2);
    tft.print("Valeur :");  
...
void loop(void) {
  ...
  if(codeur!=valeur){
    ...
    uint8_t nb=6;if(abs(valeur)<100){nb=5;}else if(abs(valeur)<10){nb=4;}
    if(valeur<0){nb++;}
    tft.setCursor(posrectx+(wrect-corpsw[textsize-1]*nb)/2+1,posrecty+1+(hrect-corpsh[textsize-1])/2);
    tft.print(valeur);
    
/*    Serial.println(hrect);
    Serial.println(posrecty);
    Serial.println(corpsh[textsize-1]);
*/    
