...
void setup() 
{
  ...
  digitalWrite(BUZZER_PIN, HIGH); // BUZZER aus (Low-Aktiv)
  
  pinMode(AUDIO_EN, OUTPUT);
  digitalWrite(AUDIO_EN, LOW);  //enable Audio
}
...
void buzz(long frequency, long length) 
{
  ...
  { // for the calculated length of time...
      ...
      digitalWrite(BUZZER_PIN, LOW); // BUZZER an
      delayMicroseconds(delayValue); // warten auf den berechneten Verzögerungswert
      digitalWrite(BUZZER_PIN, HIGH); // BUZZER aus
      delayMicroseconds(delayValue); // warten auf den berechneten Verzögerungswert
  }
}
